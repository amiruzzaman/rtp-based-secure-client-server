...
void test_header_with_csrc(void) {
    ...
    size_t fill_len = 0;
    enum rtp_status rc = rtp_header_serialize(&test_header, sizeof(header_buff), header_buff, &fill_len);
    assert(rc == STATUS_OK);
    assert(fill_len == sizeof(header_buff));

    struct rtp_header test_deserialize_header = {};
    size_t read_len = 0;
    rc = rtp_header_deserialize_pre_ext(&test_deserialize_header, fill_len, header_buff, &read_len);
    assert(rc == STATUS_OK);
